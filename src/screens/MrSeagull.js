...
export default function ChatBotScreen({ navigation }) {
  ...
  let initialPromptTemplate =
    "From now on, you are the personal assistant of me. Your task is to help me with my situation. Here is my diary of today:\n\n${userDiary}\n\n\
  ...
  async function communicateMrSeagull(messages) {
    ...
    const userDiary =
      "Today was very difficult, I felt very sad because my friend did not want to play with me :((";
    const userMood =
      "My mood record for today: intensity: 60/100 and valence: 30/100";

    // insert the prompt that conditions the bot, at the beginning of the history
    history.unshift({
      role: "user",
      content: initialPromptTemplate
        .replace("${userDiary}", userDiary)
    ...
    try {
      ...
      const response = await axios.post(
        "http://mhamzaerol.pythonanywhere.com/api",
        {
          history: history,
        }
      );
      ...
      for (let i = 0; i < 10; i++) {
        setTimeout(() => {
          setMessages([
            ...messages,
            {
              text: aiMessage.slice(0, Math.floor((messageLen * (i + 1)) / 10)),
              sender: "system",
            },
          ]);
        }, 250 * i);
